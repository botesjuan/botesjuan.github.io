...
<ul>
  <li><strong style="color:#fff">No arbitrary shell.</strong> Commands are rejected outright if they contain <code>;</code>, redirection (<code>&gt; &lt;</code>), or command substitution (<code>` </code>/<code>$(...)</code>) — those stay structurally impossible. <code>&amp;&amp;</code> (sequencing) and <code>|</code> (piping) <em>are</em> permitted, relaxed on 2026-06-19 for this single-operator context so the model can combine allowed tools in one step (e.g. <code>whatweb … &amp;&amp; sslscan …</code>, <code>curl &lt;in-scope&gt; | html2text</code>) — but every stage is still <code>shlex</code>-split into its own argv list and run directly, never via <code>sh -c</code>, and <strong style="color:#fff">each stage is independently re-validated</strong> against the full guardrail stack (allowlist, deny-flags, destructive denylist, scope) before any stage of the chain is allowed to run — one failing stage blocks the whole chain.</li>
  ...
  <li><strong style="color:#fff">Scope / egress enforcement.</strong> Every destination host in a command (URLs and bare targets) must be an <em>exact</em> match in <code>scope.yaml</code> — a parent domain does <em>not</em> authorise its subdomains, and a missing/empty file fails closed. Out-of-scope ⇒ blocked before execution. This is the hard control — but today <code>scope.yaml</code> is still a static, operator-edited file (edit + restart the orchestrator to apply). Making scope update <strong style="color:#fff">dynamically from the operator's initial prompt</strong> — e.g. stating the target web application or internal subnet as part of the engagement kick-off, instead of hand-editing the file — is an active goal, not yet built (see §05, goal 09f).</li>
...
<ol>
  ...
  <li>Tighten <code>llmctl chat</code>'s client-side proposals to the same allowlist/deny-flags granularity the sandbox path already has.</li>
